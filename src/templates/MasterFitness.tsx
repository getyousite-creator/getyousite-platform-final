                ...
                <div className="bg-[#050505] text-white min-h-screen selection:bg-lime-400 selection:text-black font-sans overflow-x-hidden">
                    ...
                    <nav className="fixed top-0 w-full z-50 bg-black/60 backdrop-blur-2xl border-b border-white/5">
                        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-xl font-black tracking-tighter uppercase italic">{blueprint?.name || "KINETIC_LAB"}</span>
                            </div>
                            <div className="hidden lg:flex items-center gap-10 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">
                                <span className="hover:text-lime-400 cursor-pointer transition-colors">Programs</span>
                                <span className="hover:text-lime-400 cursor-pointer transition-colors">Intelligence</span>
                                <span className="hover:text-lime-400 cursor-pointer transition-colors">Community</span>
                                <button
                                    onClick={() => onOpen("Join")}
                    ...
                    <section className="relative min-h-screen flex items-center justify-center pt-20">
                        ...
                        <div className="relative z-10 container mx-auto px-6 text-center space-y-12">
                            ...
                            >
                                <div className="inline-flex items-center gap-3 px-4 py-1 border border-lime-400/30 bg-lime-400/10 rounded-full mb-10">
                                    ...
                                    <span className="text-[9px] font-black uppercase tracking-[0.4em] text-lime-400">Peak Performance Engine Active</span>
                                </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                                <button className="h-20 px-14 bg-lime-400 text-black font-black uppercase tracking-widest text-[11px] hover:scale-105 transition-transform flex items-center gap-3">
                                    Initiate_Training <ChevronRight className="w-5 h-5" />
                                </button>
                                <button className="h-20 px-14 border border-white/10 text-white font-black uppercase tracking-widest text-[11px] hover:bg-white/5 transition-colors flex items-center gap-3">
                                    <Play className="w-4 h-4" /> Watch_Manifesto
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="absolute bottom-10 left-0 w-full px-12 hidden lg:flex items-center justify-between border-t border-white/5 pt-10">
                            {[
                                { label: "Performance_Index", val: "9.8/10" },
                                { label: "Client_Retention", val: "94%" },
                                { label: "Fat_oxidation_Optimization", val: "Verified" }
                            ].map((stat, i) => (
                    ...
                    <section className="py-40 px-6 container mx-auto">
                        <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-8">
                            <div>
                                <h2 className="text-6xl font-black uppercase tracking-tighter italic">Training_Protocols.</h2>
                                <p className="text-[10px] font-black uppercase tracking-[0.5em] text-lime-400 mt-6">Optimized Biological Transformations</p>
                            </div>
                        </div>
                                ...
                                >
                                    ...
                                    <div className="absolute inset-x-10 bottom-10 space-y-4">
                                        ...
                                        <div className="flex items-center gap-6 pt-4 text-xs font-bold text-zinc-400">
                                            ...
                                            <span className="flex items-center gap-2"><Users className="w-4 h-4" /> 500+ Active</span>
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
                        ...
                        <div className="flex whitespace-nowrap overflow-hidden gap-20">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="flex gap-20 animate-marquee items-center">
                                    <span className="text-8xl font-black uppercase tracking-tighter italic opacity-10">Neural_Muscle</span>
                                    <span className="w-4 h-4 rounded-full bg-lime-400" />
                                    <span className="text-8xl font-black uppercase tracking-tighter italic opacity-10">Sovereign_Athleticism</span>
                                    <span className="w-4 h-4 rounded-full bg-lime-400" />
                                    <span className="text-8xl font-black uppercase tracking-tighter italic opacity-10">Bio_Optimization</span>
                                    <span className="w-4 h-4 rounded-full bg-lime-400" />
                                </div>
                            ))}
                        </div>
                    ...
                    <footer className="py-40 px-10 flex flex-col items-center gap-16 border-t border-white/5 bg-black">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-20 w-full max-w-6xl mx-auto">
                            {[
                                { icon: Timer, label: "Live_Tracking" },
                                { icon: Scale, label: "Bio_Metrics" },
                                { icon: Trophy, label: "Empire_Lead" },
                                { icon: Activity, label: "Logic_Sync" }
                            ].map((item, i) => (
                        ...
                        <div className="text-center space-y-6">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[1em] text-zinc-800">Sovereign Performance Terminal. v4.2. Logic Guaranteed.</p>
                        </div>
                    </footer>
                </div>
            )}
